...
export const loadCartApi = () => async dispatch => {
    try {
        const response = await apiRequest(`${window.siteUrl}/?route=common/cart.info&api=true`, {
            method: 'GET',
        });
        if (response.ok) {
...
export const clearCartApi = () => async dispatch => {
    try {
        const response = await apiRequest(`${window.siteUrl}/?route=common/cart.clearCart&api=true`, {
            method: 'GET',
        });
        if (response.ok) {
            const data = await response.json();
...
export const addToCart = (product, quantity, options) => async dispatch => {
    ...
    try {
        const response = await apiRequest(`${window.siteUrl}/?route=checkout/cart.add&api=true`, {
            method: 'POST',
            headers: {
...
export const removeItemFromCart = (cart_id) => async dispatch => {
    try {
        ...
        const response = await apiRequest(
            `${window.siteUrl}/index.php?route=common/cart.removeProduct&api=true`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
