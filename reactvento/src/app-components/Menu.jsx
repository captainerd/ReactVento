...
export default function Component({ menuData, headerData }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await apiRequest(`${window.siteUrl}/?route=account/login.apilogin`, {
        method: "POST",
        headers: {
  ...
  const handleLogOut = async () => {
    try {
      const response = await apiRequest(`${window.siteUrl}/?route=account/logout`, {
        method: 'GET',
      });
        ...
        <div className="hidden md:flex flex-wrap gap-6 text-sm font-medium">
          {menuData?.map((menuItem, index) => (
            <div key={index} className="relative group">
              <a
                onClick={(e) => handleMenuClick(menuItem.href)}
                href={"#"}
