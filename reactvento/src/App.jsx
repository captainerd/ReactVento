...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await apiRequest(`${window.siteUrl}/?api=true`, {
          method: "GET",
        });
  ...
  return (
    homeData && homeData.header && homeData.menu ? (
      <Provider store={store}>

        <Router>
          <>
            <Navigation headerData={homeData.header} menuData={homeData.menu.categories} />
            <Routes>
              <Route
                path="/reactjs"
